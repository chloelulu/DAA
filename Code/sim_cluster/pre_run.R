                    ...
                    for (nSub in nSubs){
                      for (covariate.eff.mean in covariate.eff.means){
                        for (diff.otu.direct in diff.otu.directs){
                          ...
                          meta.dat <- as.data.frame(Sim.obj$X) %>% dplyr::rename(grp = V1);head(meta.dat)
...
clsapply <- function(dat, func, df=NULL, tempdir="~/project/tempZ",
  ...
  txt <- paste('
      ...
      load("dat.RData") 
                        ...
                        source("../DM1.R")
                        #source("../GMPR.R")
      pkg <- c("GMPR","phyloseq","aod","reshape","MASS","corncob","readr","DESeq2", "ALDEx2", "metagenomeSeq", "edgeR", "GUniFrac", "grDevices", "dirmult", "exactRankTests","nlme", "dplyr", "magrittr", "tidyr", "protoclust", "ggplot2", "compositions","rmutil","tibble","reticulate","dacomp","LDM","Wrench","RioNorm2")
